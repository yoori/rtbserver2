...
namespace ChannelSvcs
{
  ...
  SoftMatcher* ChannelContainer::get_matcher(
    ...
    noexcept
  {
    ...
    else
    {
      ...
      array[num]->add_entity(word, entity, matcher->trigger_type());
    }
  }
    ...
    /*throw(Exception)*/
  {
    try
    {
      ...
      {//group all unmerged triggers by trigger + lang index
        ...
        while(!matters.empty())
        {
          ...
          {//new key need to create list first to allow use trigger ref inside it
            ...
            unm_value.push_back(UnmergedItem(channel_id));
            unm_value.back().word.swap(word);
            unmerged[UnmergedKey(item.lang, unm_value.back().word.trigger)].swap(unm_value);
          }
          else
          {
            ch_it->second.push_back(UnmergedItem(channel_id));
      ...
      {//find existing matcher from container
        SoftMatcher* matcher =
          get_matcher(it->first.trigger, it->first.lang, match_word);
        if (matcher)
        {
          add_with_existing_matcher_(
            *match_chunks, it->second, matcher, match_word);
          for(auto it_un = it->second.begin();
              it_un != it->second.end(); it_un++)
          {
    ...
    /*throw(Exception)*/
  {
    ...
    while(!unmerged.empty())
    {
      ...
      {
        add.fill_lexemes(it->first.lang, parts, lexemes);
        auto num = 0;
        if(!lexemes.empty())
        {
          for(auto part_it = parts.begin();
              part_it != parts.end(); ++part_it, ++num)
          {
            if (Serialization::quoted(it->first.trigger.c_str(), num))
            {
              lexemes[num] = nullptr;
            }
          }
      ...
      SoftMatcher_var new_matcher = new SoftMatcher(
        it->first.lang,
        it->second.begin()->word.trigger);
      //trigger has moved to SoftMatcher
      ...
      //upate ns here, all matcher are new
