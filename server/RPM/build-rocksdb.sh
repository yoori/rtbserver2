...
#Source0: rocksdb- %{version}.tar.gz
...
make install INSTALL_PATH=%{buildroot}/usr
...
spectool --force -g -R --define "version $VERSION" "$ROCKSDB_SPEC_FILE" || \
  { echo "can't download rocksdb source RPM" >&2 ; exit 1 ; }

rpmbuild --force -ba --define "version $VERSION" "$ROCKSDB_SPEC_FILE" || \
