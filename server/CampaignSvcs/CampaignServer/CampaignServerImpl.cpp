...
namespace AdServer
{
  namespace CampaignSvcs
  {
    ...
    CampaignServerBaseImpl::fill_amount_distribution_(
      AdServer::CampaignSvcs::AmountDistributionInfo& amount_ditribution_info,
      const BillStatSource::Stat::AmountDistribution& amount_ditribution)
      noexcept
    {
      amount_ditribution_info.prev_days_amount.day =
        CorbaAlgs::pack_time(amount_ditribution.prev_day);
      amount_ditribution_info.prev_days_amount.amount =
        CorbaAlgs::pack_decimal(amount_ditribution.prev_days_amount);

      amount_ditribution_info.day_amounts.length(
        amount_ditribution.day_amounts.size());
      CORBA::ULong day_i = 0;
      for(auto day_it = amount_ditribution.day_amounts.begin();
        day_it != amount_ditribution.day_amounts.end();
        ++day_it, ++day_i)
      {
        amount_ditribution_info.day_amounts[day_i].day =
          CorbaAlgs::pack_time(day_it->first);
        amount_ditribution_info.day_amounts[day_i].amount =
...
*/
    ...
    CampaignServerBaseImpl::get_bill_stat() throw(
      AdServer::CampaignSvcs::CampaignServer::NotSupport,
      AdServer::CampaignSvcs::CampaignServer::NotReady)
    {
...

      {
            ...
            cmp_it != bill_stat->campaigns.end(); ++cmp_it, ++cmp_i)
        {
          ...
          fill_amount_distribution_(cmp_info.amount_distribution, cmp_it->second);
        }
      }

      {
            ...
            ccg_it != bill_stat->ccgs.end(); ++ccg_it, ++ccg_i)
        {
          ...
          fill_amount_distribution_(ccg_info.amount_distribution, ccg_it->second);
        }
      }
    ...
    void
    CampaignServerImpl::update_stat() throw(
      AdServer::CampaignSvcs::CampaignServer::NotSupport,
      AdServer::CampaignSvcs::CampaignServer::ImplementationException)
    {
    ...
    CampaignServerImpl::get_stat() throw(
      AdServer::CampaignSvcs::CampaignServer::NotSupport,
      AdServer::CampaignSvcs::CampaignServer::NotReady)
    {
      try
    ...
    AdServer::CampaignSvcs::StatInfo*
    CampaignServerProxyImpl::get_stat() throw(
      AdServer::CampaignSvcs::CampaignServer::NotSupport,
      AdServer::CampaignSvcs::CampaignServer::NotReady)
    {
    ...
    void
    CampaignServerProxyImpl::update_stat() throw(
      AdServer::CampaignSvcs::CampaignServer::NotSupport,
      AdServer::CampaignSvcs::CampaignServer::ImplementationException)
    {
      throw AdServer::CampaignSvcs::CampaignServer::NotSupport(
