...
namespace Bidding
{
  ...
  namespace
  {
    ...
    public:
      ...
      void
      add_cat_list(const StringList& cat_list, bool open_rtb = false)
      {
        for(StringList::const_iterator l_iter = cat_list.begin();
  ...
  class JsonBannerParamProcessor:
    public JsonParamProcessor<JsonAdSlotProcessingContext>
  {
  public:
    JsonBannerParamProcessor()
    {
    ...
    public JsonParamProcessor<JsonAdSlotProcessingContext>
  {
    ...
    typedef ReferenceCounting::SmartPtr<
        JsonDealParamProcessor> JsonDealParamProcessor_var;

  public:
    JsonPmpParamProcessor()
    {
      typedef ReferenceCounting::SmartPtr<
          JsonCompositeParamProcessor<Deal> >
  ...
  public:
    JsonNativeImageProcessor() :
      JsonNativeAssetProcessor<Image>(
        &Native::image_assets)      
    {
  ...
  private:
      ...
      JsonNativeCompositeParamProcessor* native_processor)
    {
      ...
      native_processor->add_processor(
          ...
          JsonNativeParamProcessor_var(
            new JsonArrayParamProcessor<Native>(
              asset_processor)));
    }
  ...
  public:
    JsonImpParamProcessor()
    {
      ...
      JsonAdSlotParamProcessor_var banner_processor =
    ...
    /*throw(InvalidParamException, Exception)*/
  {
    ...
    init_request_param_(request_params, request_info);
    ...
    */
    ...
    if(bid_request.has_url())
    {
      ...
      fill_by_referer_(request_params, request_info.search_words, url);
    }
    ...
    for(int slot_i = 0; slot_i < bid_request.adslot_size(); ++slot_i)
    {
      ...
      {
        std::ostringstream res_cat_ostr;
        res_cat_ostr << adslot.excluded_sensitive_category(cat_i);
        ad_slot_request.exclude_categories[res_cat_i] << res_cat_ostr.str();
        ++res_cat_i;
      }
      ...
      {
        std::ostringstream res_cat_ostr;
        res_cat_ostr << adslot.excluded_product_category(cat_i);
        ad_slot_request.exclude_categories[res_cat_i] << res_cat_ostr.str();
        ++res_cat_i;
      }
    ...
    noexcept
  {
    ...
    if(source_it != sources_.end())
    {
      ...
      request_params.context_info.enabled_notice = source_it->second.vast_enabled_notice;
    }
  }
    ...
    noexcept
  {
    ...
    if(source_it != sources_.end())
    {
      ...
      request_params.context_info.enabled_notice = source_it->second.enabled_notice;
    ...
    /*throw(InvalidParamException, Exception)*/
  {
    ...
    {
      fill_by_ip_(request_info, request_params, context.ipv6);
    }
    else
    {
      fill_by_ip_(request_info, request_params, context.ip);
    }
          ...
          debug_sizes.empty() && default_debug_size.empty()))
      {
        ...
        if (!slot_it->deals.empty())
        {
          ...
          std::string currency_code(
            slot_it->min_cpm_price_currency_code.empty() ? "USD":
    ...
    context.ifa = normalize_ifa(context.ifa);
    ...
    add_special_keywords_(keywords, request_info, context.app_id);
  }
    ...
    /*throw(InvalidParamException, Exception)*/
  {
...

    {
      ...
      fill_by_referer_(request_params, request_info.search_words, referer);
  ...
  void
  RequestInfoFiller::fill_by_user_agent_(
    AdServer::CampaignSvcs::CampaignManager::RequestParams& request_params,
    RequestInfo& request_info,
    ...
    noexcept
  {
    static const char* FUN = "RequestInfoFiller::fill_by_user_agent_()";
  ...
  void
  RequestInfoFiller::fill_by_referer_(
    AdServer::CampaignSvcs::CampaignManager::RequestParams& request_params,
    ...
    /*throw (eh::Exception)*/
  {
    ...
    try
    {
      ...
      {
        std::string search_words;//TODO: ignory them?
        fill_additional_url_(
          request_params,
          search_words,
          context.site_referer);
      }
    }
    catch(const eh::Exception&)
    {//ignory bad url
    ...
    try
    {
      if(referer.url().empty())
      {
        referer = context.site_rereferer;
      }
      else
      {
        std::string search_words;//TODO: ignory them?
  ...
  void
  RequestInfoFiller::init_adslot_(
  ...
  void
  RequestInfoFiller::init_request_param_(
    AdServer::CampaignSvcs::CampaignManager::RequestParams& request_params,
